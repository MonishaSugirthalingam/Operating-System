...
{
    int n,i;
    int copyID[10],copyAt[10],copyBt[10],copyPt[10];
    ...
    //copy of id,priority,arrival and bursttime
    for(int i=0;i<n;i++)
    {
        copyID[i]=a[i].id;
        copyAt[i]=a[i].at;
   ...
   while(!burst(a,n))
    ...
    for(i=0;i<n;i++)
    {
        printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",copyID[i],copyPt[i],copyAt[i],copyBt[i],ct[i+1],wt[i],tut[i]);
    }
